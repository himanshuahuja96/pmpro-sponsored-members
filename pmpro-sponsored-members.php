...
function pmprosm_pmpro_registration_checks_sponsored_accounts( $okay ) {
	...
	if( $num_new_accounts > 0 && ( ($pmprosm_values['children_hide_username'] !== true && count( $unique_usernames ) < $num_new_accounts) || (count( $unique_emails ) < $num_new_accounts && $pmprosm_values['children_hide_email'] !== true ) || count( $passwords ) < $num_new_accounts ) ) {
